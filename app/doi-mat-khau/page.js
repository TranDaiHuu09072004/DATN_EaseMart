...
export default function ChangePassword() {
  ...
  const handleChangePassword = async (e) => {
    ...
    if (currentPassword !== storedPassword) {
      Swal.fire("Lỗi", "Bạn đã nhập sai mật khẩu hiện tại!", "error");
      return;
    }
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Cập nhật mật khẩu trong localStorage và đăng xuất người dùng
        localStorage.setItem("currentPassword", newPassword);
        localStorage.removeItem("user");
        setTimeout(() => {
          window.location.href = "/dangnhap"; // Điều hướng về trang đăng nhập
        }, 2000);
      } else {
