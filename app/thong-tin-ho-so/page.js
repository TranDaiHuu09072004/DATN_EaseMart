  ...
  const fetchCustomerData = async () => {
    try {
      const response = await fetch("https://trandainghia.id.vn/api/customers", {
        method: "GET", // Thay đổi thành 'GET' nếu API hỗ trợ
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`, // Đảm bảo token được định nghĩa
        },
        body: JSON.stringify({ email }), // Nếu API yêu cầu email, giữ lại phần này
      });
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className={cx("phone_input")}>
              ...
              <input
                type="tel"
                name="phone"
                value={userData.phone} // Ensure userData is used hereonChange={handleChange}
